...
public class UserServiceImpl implements UserService{
	...
	//SecurityConfig 클래스에서 bean 으로 등록된 객체 주입 받기 
	...
	public void updatePassword(UserDto dto) {
		...
		UserDto dto1 = dao.getPrePwd(userName);
		//3. 입력한(암호화 되지 않은 (구)비밀번호) 와 일치하는지 비교 해서
		boolean isTrue = BCrypt.checkpw(dto1.getPassword(), dto.getPassword());
		//4. 만일 일치하지 않으면 Exception 을 발생시킨다. (throws)
		if(!isTrue) {
