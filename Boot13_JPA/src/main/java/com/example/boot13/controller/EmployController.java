...
//									.stream().map(EmpDeptDto::toDto).toList();
	...
	public String list3(Model model, @RequestParam(defaultValue = "1") int pageNum) {
		...
		//원하는 페이지정보를 담고 있는 Pageable객체를 얻어내서
		...
		//pageNum 에 해당하는 사원 목록을 Model에 담는다
		model.addAttribute("list", list);
...
//		List<Emp> empList = empRepo.getList(2000);
	...
	public String list2(Model m, int pageNum){
		//한 페이지에 몇개씩 표시할 것인지
		final int PAGE_ROW_COUNT=5;
		Sort sort = Sort.by(Sort.Direction.ASC, "empno"); // 사원번호에 대해 오름차순 정렬
		/*
	...
	public String detail(int num, Model m) {
		...
		Dept dept = deptRepo.findById(num).get(); // 어떠한 동작 후 get()을 호출하여 Dept 타입을 얻어낼 수 있다.
